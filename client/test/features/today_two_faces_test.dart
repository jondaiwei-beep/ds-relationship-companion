...
void main() {
  ...
  group('Today has two faces', () {
      ...
      await pump(tester, const TodayView(
        ...
        expectations: [
          TodayItem(occurrenceId: 'o1', title: 'Prepare the evening space',
              purpose: 'Something small.', state: 'ACTIVE'),
        ],
      ));
    ...
    testWidgets('the direction-giving side is offered the way in',
      ...
      expect(find.text('2 things need your response.'), findsOneWidget);
      await tester.tap(find.text('2 things need your response.'));
      await tester.pumpAndSettle();
      expect(opened, 1);
    });
    ...
    testWidgets('both faces can appear at once', (tester) async {
      ...
      await pump(tester, const TodayView(
        ...
        expectations: [
          TodayItem(occurrenceId: 'o1', title: 'Evening check-in',
              purpose: 'A few words.', state: 'ACTIVE'),
        ],
      ));

      expect(find.text('One thing needs your response.'), findsOneWidget);
      expect(find.text('Evening check-in'), findsOneWidget);
    });

    testWidgets('a partner\'s words outrank the routing row', (tester) async {
      await pump(tester, TodayView(
      ...
      final door = tester.getTopLeft(find.text('2 things need your response.'));
      expect(words.dy < door.dy, isTrue,
    ...
    testWidgets('it says a person is waiting, never that something is late',
      ...
      for (final banned in [
        'overdue', 'late', 'missed', 'pending', 'due', 'backlog', 'tasks',
      ]) {
        expect(allText(tester).toLowerCase().contains(banned), isFalse,
