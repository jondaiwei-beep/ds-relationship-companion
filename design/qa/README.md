...
`reference/token-freeze-b2-v1-board.png` is the canonical semantic color, surface, state and geometry board. `reference/token-freeze-b2-v1-validation.json` records required contrast and geometry checks; regenerate both with `design/qa/scripts/render-token-freeze.cjs` after running the token binding generator.

`reference/today-b3-state-family-board.png` is the canonical Today Revision 2 comparison board. It places the default three-priority state, eight-item expansion, preserved ritual focus and loading/empty/error/offline/authorization/role/solo variants in one review surface. `reference/today-b3-state-family-validation.json` records source/preview paths, token freeze and semantic Asset IDs; regenerate both with `design/screens/SCR-01-today/candidates/rev-2/render-today-b3.cjs`.
