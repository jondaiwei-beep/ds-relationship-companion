...
| Preserved source | `04 Core Relationship/13-dynamic-overview.webp` |

## 1. Product contract

- Requirement IDs: `REQ-EXPECT-001`, `REQ-PAUSE-001`, `REQ-PRIVACY-001`
- Flow IDs: `FLOW-PAUSE-001`
- Job to be done: Understand the current relationship rhythm and access essential adjustments.
- Entry and preconditions: Authorized member opens Dynamic.
- Roles and permissions: Both members see current shared structure; agency and privacy entry points remain available.
- Business logic and state transitions: Show partner/role context, current rituals/recurring structure, Pause/Resume and basic settings; remove Agreement prominence.
- Acceptance criteria: Core Beta hierarchy is not a governance dashboard and all displayed state matches server truth.
...
- remove Agreement from Core Beta main page
- show current rhythm, partner context, pause/resume, basic settings
...
- [ ] Reference-size implementation render stored under `design/qa/implementation/SCR-13/`
