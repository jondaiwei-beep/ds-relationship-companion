...
| Preserved source | `06 Tasks and Agreement Review/20-create-task.webp` |

## 1. Product contract

- Requirement IDs: `REQ-EXPECT-001`
- Flow IDs: None
- Job to be done: Create one low-load Task or Ritual definition.
- Entry and preconditions: Authorized direction-giving member chooses to add to the current rhythm.
- Roles and permissions: Creator can assign only within allowed current Dynamic context; partner agency remains intact.
- Business logic and state transitions: Simplify title/purpose/assignee/schedule/visibility; no Proof/points/punishment; starter suggestions may reduce blank canvas.
- Acceptance criteria: Creation takes little mental load and history is not rewritten by later edits.
...
- simplify creation mental load
- remove Proof/photo option from Core Beta
...
- [ ] Reference-size implementation render stored under `design/qa/implementation/SCR-20/`
