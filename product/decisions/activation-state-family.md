...
**1. 前三步并非完全无网络状态。** 我说「草稿在客户端,所以 N/A」——
但**第 3 步的 Continue 会触发** `POST /v1/dynamics` 加 `GET starter-rhythm`。
草稿是本地的,**跳转不是**。所以 SCR-08 的 Continue 需要 loading / offline /
retry / error,和第 4 步一样。SCR-31 与 SCR-07 的 Continue 才是真的纯本地。

**2. 幂等键按尝试次数持有是错的,应该按请求内容。** 服务端把键绑定到
**具体的请求体**。所以:同一个键重试一个**已修改**的草稿是冲突;
而成功后清除键,则意味着返回上一步再提交会创建第二个 Dynamic。
现在按请求内容做指纹 —— 同一份草稿永远是同一次尝试(包括响应丢失的成功),
改过的草稿诚实地是另一个请求。

**3. `mode` 在仓库层被硬编码成 `COUPLE`。** 意味着 SCR-07 上选
「For myself」**完全没有效果**,照样创建一个 couple Dynamic。已修。

另加:**时区现在校验形状**。`REQ-TIME-001` 要求 IANA 名称,
而 `+02:00` 能通过所有类型检查、然后在夏令时切换时移动关系日 ——
故障几个月后才出现,且看起来像排期 bug。

## 需要产品决策的缺口(不是客户端能修的)

| 缺口 | 事实 |
|---|---|
| **Long-distance / Together** | SCR-08 收集它,**服务端没有任何地方存它**。不是客户端 bug,是数据模型缺一列。要么建模,要么从设计里去掉 |
| **Boundaries & preferences** | 设计稿只有一个入口行,二级页面未设计,命令层也无对应 |
| **Add another expectation** | API 只支持 `includeSecondExpectation` 一个布尔,设计稿的「+」暗示可加任意条 |
| **每条 Replace 的选择界面** | 设计稿有 Replace 链接,替换成什么的界面未设计 |
| **`assigneeUserId`** | 启动节奏必须指定给谁,**四张设计稿里没有任何地方选这个**。目前只能默认给伴侣 —— 需要确认这是有意的 |
| **`dayBoundaryMinutes`** | SCR-34 的题目,激活时用默认值 0 |

## 一处我保留不同意见

Codex 说「`POST /v1/dynamics` 先于节奏写入,所以『节奏是第一个变成真实的东西』
是错的」。**字面上它对**,我改了那句话的措辞。

但产品上我认为区分仍然成立:一个没有节奏、没有伴侣的 Dynamic 行里
不包含任何关系内容,它更像一个尚未使用的容器。真正开始的是节奏。
不过既然它确实写了库,就不该说成「什么都没发生」——
状态族里第 3 步的 Continue 因此需要完整恢复态,这一点 Codex 是对的。
